...
import { ArrowLeft, Sparkles, Code, CheckCircle, Lightbulb } from 'lucide-react';
import Markdown from 'react-markdown';
import { geminiTipsMarkdown } from '../config';

interface Props {
  onBack: () => void;
}

export const GeminiTipsPage: React.FC<Props> = ({ onBack }) => {
  return (
      ...
      <div className="bg-white rounded-[8px] p-6 md:p-10 shadow-sm border border-[#e8e8e8]">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="text-2xl md:text-3xl font-bold text-[#222]">
            Trik Rahasia Menguasai AI
          </h1>
        </div>

        <div className="prose prose-purple max-w-none prose-pre:bg-[#282c34] prose-pre:text-green-400">
          <Markdown>{geminiTipsMarkdown}</Markdown>
        </div>
      </div>
    </div>
  );
